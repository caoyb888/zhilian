...
const STATUS_TABS = [
  ...
  { label: '已下架', value:  3, dot: 'bg-stone-400' },
]
...
const STATUS_CONFIG: Record<number, { label: string; bar: string; badge: string; text: string }> = {
  0: { label: '待审核', bar: 'bg-amber-400',   badge: 'bg-amber-50 text-amber-700 border-amber-200',   text: 'text-amber-600' },
  1: { label: '已通过', bar: 'bg-emerald-500', badge: 'bg-emerald-50 text-emerald-700 border-emerald-200', text: 'text-emerald-600' },
  2: { label: '已拒绝', bar: 'bg-red-400',     badge: 'bg-red-50 text-red-700 border-red-200',         text: 'text-red-600' },
  3: { label: '已下架', bar: 'bg-stone-300',   badge: 'bg-stone-100 text-stone-600 border-stone-200',  text: 'text-stone-500' },
}

const TYPE_CONFIG: Record<string, { icon: typeof Package; chip: string }> = {
  PRODUCT:    { icon: Package, chip: 'bg-blue-50 text-blue-700 border-blue-200' },
  TECHNOLOGY: { icon: Zap,     chip: 'bg-violet-50 text-violet-700 border-violet-200' },
    ...
    <>
      ...
      {pendingAction === 'withdraw' && (
        <ConfirmModal
          ...
          confirmLabel="确认撤回"
          isPending={isPending}
          onConfirm={() => withdrawMutation.mutate(item.id, { onSuccess: () => setPendingAction(null) })}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="relative overflow-hidden rounded-2xl shadow-lg">
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #065f46 0%, #0d9488 100%)' }} />
        ...
        <div className="absolute -right-8 -top-8 h-36 w-36 rounded-full opacity-15" style={{ background: 'radial-gradient(circle, #34d399, transparent)' }} />
        <div className="relative flex items-center justify-between gap-4 px-6 py-5">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-white/15 ring-1 ring-white/25 backdrop-blur-sm">
              <Icon icon={Package} size={22} className="text-white" />
            </div>
            <div>
              <h1 className="text-xl font-bold tracking-tight text-white">我的资源</h1>
              <p className="mt-0.5 text-sm text-emerald-100/75">
                {total > 0 ? `共 ${total} 条资源` : '管理已发布的供给资源'}
              </p>
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-2">
          {records.map((r, idx) => (
            <ResourceCard key={r.id} item={r} index={(page - 1) * PAGE_SIZE + idx + 1} />
          ))}
        </div>
      )}
      ...
      {hasRejected && (
        <div className="flex items-start gap-2.5 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm text-amber-700">
          ...
          <p><span className="font-semibold">已拒绝的资源：</span>请删除后重新发布，或联系管理员了解详情。</p>
        </div>
      )}
    </div>
  )
