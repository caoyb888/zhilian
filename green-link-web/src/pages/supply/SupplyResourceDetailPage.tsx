...
function ContactCard({ memberId, contactVisible, isLoggedIn, onLoginRequest }: ContactCardProps) {
  const { data: member, isLoading } = useMemberDetail(
    contactVisible === 1 && isLoggedIn ? memberId : null,
  )
    ...
    return (
      <div className="min-h-screen flex flex-col bg-theme-bg">
        ...
        <div className="flex flex-1 items-center justify-center">
          <ErrorState
            title="资源不存在或已下架"
            description="您访问的资源可能已被删除或暂时无法查看"
            action={
              <button
